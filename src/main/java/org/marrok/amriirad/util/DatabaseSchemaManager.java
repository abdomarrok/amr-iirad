...
public class DatabaseSchemaManager {
    ...
    public static void runMigrations() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            // Seed default roles
            stmt.execute(
                    "INSERT IGNORE INTO role (id, name, description, privilege_level, is_protected) VALUES (1, 'ADMIN', 'مدير النظام', 100, TRUE)");
            stmt.execute(
                    "INSERT IGNORE INTO role (id, name, description, privilege_level, is_protected) VALUES (2, 'OPERATOR', 'مستخدم عادي', 50, FALSE)");
            ...
            try {
                ...
                // Fill nulls if any (legacy data)
                stmt.execute("""
                            UPDATE budget_chapter bc 
                            SET bc.fiscal_year_id = (SELECT id FROM fiscal_year ORDER BY year_label DESC LIMIT 1)
            ...
            stmt.execute("""
                        INSERT IGNORE INTO budget_chapter (id, code, label_ar, label_fr, fiscal_year_id, level) VALUES
                        (1, '01', 'الإيرادات الجبائية', 'Recettes Fiscales', 1, 1),
                        (2, '01.01', 'الضرائب المباشرة', 'Impôts Directs', 1, 2),
                        (3, '02', 'إيرادات الأملاك', 'Produits du Domaine', 1, 1);
                    """);
            ...
            if (!hasActive) {
                try (var rsCount = stmt.executeQuery("SELECT id FROM fiscal_year ORDER BY year_label DESC LIMIT 1")) {
                    if (rsCount.next()) {
                        ...
                        stmt.execute(
                                "INSERT INTO fiscal_year (year_label, is_active) VALUES ('" + currentYear + "', TRUE)");
    ...
    private static void insertPermission(Statement stmt, String code, String type, String category, String desc)
            throws SQLException {
        stmt.execute(String.format(
